...
class HyperSegHInference:
    ...
    def load_checkpoint(self, checkpoint_path):
        ...
        audit_load(self.PointFeatureEnhancer, enhancer_state_dict, 'enhancer')
        audit_load(self.decoder, decoder_state_dict, 'decoder')
        audit_load(self.seg_head, seg_head_state_dict, 'seg_head')
        
        print(f"Checkpoint loaded: {checkpoint_path}")
    
    @torch.no_grad()
    def predict_mask(self, point_feat, point_coords, point_color, prompt_idx, threshold=0.5, continuous_scale=None):
        if self.hyperseg is not None:
            if continuous_scale is None:
                raise ValueError('HyperSeg-H requires explicit scale or hierarchy control')
            from hyperseg_h.data import unit_sphere
            points = unit_sphere(point_coords.to(self.device).float()).unsqueeze(0)
            probs = self.hyperseg(points, torch.tensor([prompt_idx], device=self.device),
                                 torch.tensor([continuous_scale], device=self.device),
                                 point_feat.to(self.device).unsqueeze(0))[0].float().cpu().numpy()
            return probs > threshold, float(probs.max()), probs
        batch_size = 1
    ...
    def _(client: viser.ClientHandle) -> None:
        ...
        with params_section:
            ...
            scale_slider = client.gui.add_slider(
                "🎚️ Hierarchy g (coarse → fine)" if model.control_signal == 'hierarchy' else "🎚️ Segmentation Scale",
                min=0.0,
                max=1.0,
                step=0.01,
                initial_value=0.3,
                hint=("Path-relative g: 0 = coarse, 1 = fine" if model.control_signal == 'hierarchy'
                      else "Scale s: 0 = small parts, 1 = large parts")
            )
            if model.hyperseg is not None:
                use_scale_checkbox.disabled = True
