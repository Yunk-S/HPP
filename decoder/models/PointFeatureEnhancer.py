...
class TransformerBlock(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=True, 
                 attn_drop=0., proj_drop=0., drop_path=0., batch_first=False):
        super().__init__()
        self.norm1 = nn.LayerNorm(dim)
        self.attn = nn.MultiheadAttention(dim, num_heads, dropout=attn_drop, bias=qkv_bias, batch_first=batch_first)
...
class PointFeatureEnhancer(nn.Module):
    def __init__(self, feature_dim=448, pos_embed_dim=384, feature_proj_dim=384,
        ...
        if compatibility_mode not in ("official-compatibility", "batch-first-corrected"):
            raise ValueError(f"Unknown compatibility_mode: {compatibility_mode}")
        ...
        self.transformer_blocks = nn.ModuleList([
            TransformerBlock(
                ...
                proj_drop=dropout,
                drop_path=dropout,
                batch_first=compatibility_mode == "batch-first-corrected"
...
def make(cfg):
    return PointFeatureEnhancer(
        ...
        compatibility_mode=cfg.get('compatibility_mode', 'official-compatibility')
    )
