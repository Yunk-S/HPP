...
class CollateNpy:
    def __init__(self, num_points=10000, is_training=True, normalization="legacy-standardize"):
    ...
    def __call__(self, batch_paths):
        return load_npy_to_dict(
            batch_paths, is_training=self.is_training, num_points=self.num_points, normalization=self.normalization
        )
...
def load_npy_to_dict(file_paths, is_training=True, num_points=10000, normalization="legacy-standardize"):
    input_dict = {
        "coord": [],
        "feat": [],
        "label": [],
    ...
    for file_path, feat_path in file_paths:
        try:
            ...
            if normalization == "unit-sphere":
                coord_tensor = unit_sphere(coord_tensor)
            elif normalization == "legacy-standardize":
...
def make(config, rank, world_size):
    ...
    dataloader = DataLoader(
        ...
        collate_fn=CollateNpy(num_points=num_points, is_training=True,
                              normalization=config.dataset.get("normalization", "legacy-standardize")),
...
# Shared true-hierarchy path (requires explicit ancestor-chain annotations).
import sys
from pathlib import Path
_ROOT = Path(__file__).resolve().parents[3]
if str(_ROOT) not in sys.path:
    sys.path.insert(0, str(_ROOT))
from hyperseg_h.data import (HierarchyDataset, ObjectBalancedSampler, collate_hierarchy,
                            unit_sphere, boundary_prompt, exclude_overlap)
