...
def main(rank, world_size, cli_args, extras):
    ...
    if rank == 0:
        ...
        if os.path.exists(config.code_dir):
            raise FileExistsError(f"Refusing to overwrite existing code snapshot: {config.code_dir}")
        os.makedirs(config.code_dir, exist_ok=False)
...
if __name__ == "__main__":
    if "--hyperseg" in sys.argv:
        from pathlib import Path
        sys.path.insert(0, str(Path(__file__).resolve().parents[3]))
        from hyperseg_h.cli import main as hyperseg_main
        args = sys.argv[1:]
        args.remove("--hyperseg")
        hyperseg_main(args)
        sys.exit(0)
